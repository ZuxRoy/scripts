...
connect_wifi() {
    ...
    choice=$(echo -e "$toggle\n$wifi_list" | uniq -u | rofi -dmenu -i -selected-row 1 -theme-str 'entry { enabled: false;} prompt { enabled: false; } window { padding: 0px; }')
...
if [ "$flag" -eq 1 ]; then
    ...
    choice=$(echo -e "$toggle" | rofi -dmenu -i -selected-row 1 -l 1 -theme-str 'entry { enabled: false;} prompt { enabled: false; } window { padding: 0px; }')
    if [[ "$choice" = "$toggle" ]]; then
        nmcli radio wifi on
        toggle="󰖪  Disable Wi-Fi"
